# ----------------------------------------------------------------------------
#	simulated_single_rawrad.py
#
#	Copyright 2021 Daniel Tisza
#	MIT License
#
#	Take only one raw and radiance image when setting requested
#   exposure time and selecting requested wavelength
#
#	Version 2022.07.15  07:15
# ----------------------------------------------------------------------------
...
if argc == 1 or argc == 2:
    exposureTime = "60000"
    wavelength = "590"
    print("No exposure time or wavelength argument given! Using defaults 60000 and 543")
else:
    exposureTime = sys.argv[1]
    wavelength = sys.argv[2]
    print("Exposure time given as argument: ", exposureTime)
    print("Wavelength given as argument: ", wavelength)
...
print("Exposure time converted to string: ", str(wavelength))
...
class HSI:
    ...
    def capture_cube(self, *, selectors=None):
        ...
        with self.camera:
            ...
            intWavelength = int(wavelength)

            for idx in tqdm(dataset[c.image_index].values):

                # Check if wavelength matches requested wavelength
                #print('')
                #print('Wavelength data for calibration file:')
                #print(dataset[c.wavelength_data])

                wavelengtharray = dataset[c.wavelength_data].isel(index=idx)

                #print('')
                #print('Wavelength data for this setpoint setting (calibration file row):')
                #print(wavelengtharray)

                intWavelength0 = int(wavelengtharray[0])
                intWavelength1 = int(wavelengtharray[1])

                #if wavelength not matching, then continue
                #print('')
                #print('intWavelength0:')
                #print(intWavelength0)
                #print('')
                #print('intWavelength1:')
                #print(intWavelength1)

                if intWavelength != intWavelength0 and intWavelength != intWavelength1:
                    continue
                else:
                    print('')
                    print('Requested wavelength was found on calibration file row: ' + str(idx))
...
#hsi.read_calibration_file('images_nilered_coarse1.txt')
#hsi.read_calibration_file('led_set_all_calib.txt')
hsi.read_calibration_file('led_set_all_calib_led_off.txt')

input("Set dark reference and press a key to take images")
print('Capturing dark reference')

# TEST: Can take images without dark reference?
...
#
#int('')
#int('Start saving raw CFA images')
#r x in range(0, len(raw.index)):
#
...
#
#int('')
#int('Start saving radiance images')
#r x in range(0, imagelastindex):
#
